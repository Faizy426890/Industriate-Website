      ...
      <div className="relative mx-auto max-w-7xl px-5 lg:px-8 py-16 lg:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12">
          <div className="lg:col-span-4">
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed text-white/50 max-w-sm">
              Staffing, certification, compliance and immigration coordination — all in one platform.
            </p>

            <div className="mt-8 space-y-3 text-[13.5px]">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-8">
            {FOOTER_NAV.map((column) => (
              <div key={column.title}>
                <h4 className="text-[11.5px] font-semibold tracking-[0.18em] uppercase text-white/45 mb-4">
                  {column.title}
                </h4>
                <ul className="space-y-2.5">
                  {column.links.map((link) => (
                    <li key={link.href}>
                      <Link
                        href={link.href}
                        className="inline-block text-[14px] text-white/75 hover:text-white transition-colors duration-300"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="mt-14 pt-8 border-t border-white/10">
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-5 lg:p-6 flex flex-col md:flex-row md:items-start gap-4">
            <div className="flex-none w-9 h-9 rounded-md bg-white/10 grid place-items-center text-[var(--color-accent-300)]">
              <ShieldCheck size={16} />
            </div>
            <div className="flex-1">
              ...
              <p className="mt-1.5 text-[12.5px] leading-relaxed text-white/60">
                ...
                immigration attorneys. Our role is limited to staffing services and non-legal
        ...
        <div className="mt-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <p className="text-[11px] tracking-[0.16em] uppercase text-white/35">
  ...
  const content = (
    <span className="flex items-start gap-3">
      ...
      <span className="flex flex-col">
        <span className="text-[10.5px] font-semibold tracking-[0.16em] uppercase text-white/40">{label}</span>
