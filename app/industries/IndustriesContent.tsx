...
const INDUSTRY_ICONS = [<Factory size={20} key="f" />, <Stethoscope size={20} key="s" />, <Wrench size={20} key="w" />, <Truck size={20} key="t" />];
...
export function IndustriesContent() {
  ...
  return (
    <>
      ...
      {pg.industries.map((industry, idx) => (
            ...
            <div className={`grid lg:grid-cols-12 gap-12 items-center ${idx % 2 === 1 ? 'lg:[&>*:first-child]:order-2' : ''}`}>
              ...
              <div className="lg:col-span-6">
                ...
                <Reveal delay={0.15}>
                  ...
                  >
                    {pg.requestWorkforce.replace('{name}', industry.name.toLowerCase())}
